...
C                                                                      C
      SUBROUTINE CKABS
C
C     CKLIB is the library of gas-phase kinetics subroutines for the
C     collection of codes known as CHEMKIN.  The acronym CHEMKIN is a
C     registered Trademark.
C///////////////////////////////////////////////////////////////////////
C
C     CHEMKIN-II  VERSION 4.5
C
...
C
     ...
     1             RCKWRK(NcPA), T_A, ICKWRK(IcNS), ICKWRK(IcNU),
...
C
      ...
      DO 150 M = 1, NMM
         ...
         ELSE
!HZ comment the following code to avoid writing conflicts with multi-cores            
!HZ            WRITE (LOUT,'(A)')      
            WRITE (0,'(A)')
...
C
      CALL CKRATX (NII, NKK, MXSP, MXTB, T(1), RCKWRK(NcK1), 
     1             ICKWRK(IcNU), ICKWRK(IcNK), NPAR+1, RCKWRK(NcCO),
     2             NFAL, ICKWRK(IcFL), ICKWRK(IcFO), ICKWRK(IcKF), NFAR,
     3             RCKWRK(NcFL), NTHB, ICKWRK(IcTB), ICKWRK(IcKN),
     4             RCKWRK(NcKT), ICKWRK(IcKT), RCKWRK(NcKF),
