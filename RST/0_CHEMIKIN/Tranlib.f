...
C         loop 1600 in MCLMDT were both in correct.
C
      RETURN
      END
C---------------------------------------------------------------------
...
C
   ...
   15 FORMAT(
     ...
     2/'           CHEMKIN-II Version 3.1, March 1994',
C*****precision > double
     3/'           DOUBLE PRECISION')
C*****END precision > double
C*****precision > single
...
C
  ...
  400       CONTINUE
     ...
     1              * (XL0000(I,J)-XL0000(I,I)) /(25.0 * P * WT(J) )
  500 CONTINUE
C
      RETURN
      END
...
C
     ...
     3                     ( 7.5*WT(J)**2 + 6.25*WT(K)**2 -
     4                       3.0*WT(K)**2 * BSTAR(I,K) )  -
     5                4.0*WT(J)*WT(K) * ASTAR(I,K) *
      ...
      DO 1650 I = 1, KK
         ...
         DO 1550 K = 1, KK
            IF (I .NE. K) THEN
               SUM = SUM + XX(I)*XX(K) /
     1                            ( (WT(I)+WT(K))**2 * BINDIF(I,K) ) *
     3                   ( ( 7.5*WT(I)**2 + 6.25*WT(K)**2 -
     4                       3.0*WT(K)**2 * BSTAR(I,K) )  +
     5                4.0*WT(I)*WT(K) * ASTAR(I,K) *
     7                       ( ONE + PIFAC *
     8                     (CROTOR(I)/XI(I) + CROTOR(K)/XI(K)) ) )
            ENDIF
 1550    CONTINUE
      ...
      DO 2200 I = 1, KK
         IF (LIN(I) .NE. 0) THEN
            ...
            DO 2100 K = 1, KK
               ...
               IF (I .NE. K) THEN
                  SUM = SUM +
     1                  12.0 * XX(I)*XX(K) * WT(I) * ASTAR(I,K) *
     2                           CROTOR(I) /
     3                  ( PIFAC * CINTOR(I) * WT(K) * BINDIF(I,K)
     4                          * XI(I)  )
               ENDIF
 2100       CONTINUE
            XL(NN, NN) = - 8.0 * WT(I) * XX(I)**2 * CROTOR(I) /
     1                  ( PIRU * CINTOR(I)**2 * VIS(I) * XI(I) ) -
     2                PFAC * SUM / CINTOR(I)
         ENDIF
 2200 CONTINUE
C
...
C
C*****precision > double
      DOUBLE PRECISION FUNCTION DELTA (I,K)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
C      REAL FUNCTION DELTA (I,K)
C      IMPLICIT REAL (A-H, O-Z), INTEGER (I-N)
C*****END precision > single      
C
